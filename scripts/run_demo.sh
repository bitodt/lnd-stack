...
# Get Alice address
ALICEPOD=$(kubectl get pods -l app="$ALICEAPPNAME" -o jsonpath='{.items[0].metadata.name}')
ALICEADDR=$(kubectl exec $ALICEPOD -- lncli --network=simnet newaddress np2wkh | jq -r '.address')
...
sleep $SMALLSTEP
kubectl patch secret $BTCDAPPNAME-secret --type='json' -p="[{\"op\": \"replace\", \"path\": \"/data/miningaddress\", \"value\": \"$ALICEADDR64\"}]"
# Restart btcd pod to get new mining address
echo Restarting btcd...
sleep $SMALLSTEP
kubectl scale --replicas=0 deployment.apps/$BTCDAPPNAME
sleep $BIGSTEP
kubectl scale --replicas=1 deployment.apps/$BTCDAPPNAME
echo Restart successful...
sleep $BIGSTEP
BTCDPOD=$(kubectl get pods -l app=$BTCDAPPNAME -o jsonpath='{.items[0].metadata.name}')
# Activate SegWit
echo Generating blocks to activate SegWit...
sleep $SMALLSTEP
kubectl exec $BTCDPOD -- /start-btcctl.sh generate 401
kubectl exec $BTCDPOD -- /start-btcctl.sh getblockchaininfo | grep -A 1 segwit
sleep $BIGSTEP

# Check Alice balance
echo Alice wallet balance...
sleep $SMALLSTEP
kubectl exec $ALICEPOD -- lncli --network=simnet walletbalance

# Get Bob pubkey
BOBPOD=$(kubectl get pods -l app="$BOBAPPNAME" -o jsonpath='{.items[0].metadata.name}')
BOBPUBKEY=$(kubectl exec $BOBPOD -- lncli --network=simnet getinfo | jq -r '.identity_pubkey')
BOBIP=$BOBAPPNAME-svc

# Connect Alice to Bob node
echo Connecting Alice to Bob...
sleep $SMALLSTEP
kubectl exec $ALICEPOD -- lncli --network=simnet connect $BOBPUBKEY@$BOBIP
# List Alice peers
echo Listing Alice peers...
sleep $SMALLSTEP
kubectl exec $ALICEPOD -- lncli --network=simnet listpeers
# List Bob peers
echo Listing Bob peers...
sleep $SMALLSTEP
kubectl exec $BOBPOD -- lncli --network=simnet listpeers

# Open channel from Alice to Bob
sleep $SMALLSTEP
echo Opening channel from Alice to Bob...
sleep $SMALLSTEP
kubectl exec $ALICEPOD -- lncli --network=simnet openchannel --node_key=$BOBPUBKEY --local_amt=1000000
sleep $BIGSTEP
...
sleep $SMALLSTEP
kubectl exec $ALICEPOD -- lncli --network=simnet listchannels

# Bob creates an invoice
echo Bob creates an invoice...
sleep $SMALLSTEP
PAYREQ=$(kubectl exec $BOBPOD -- lncli --network=simnet addinvoice --amt=10000 | jq -r '.payment_request')

# Alice pays the invoice. --force added to skip confirmation
echo Alice pays the invoice...
sleep $SMALLSTEP
kubectl exec $ALICEPOD -- lncli --network=simnet sendpayment --force --pay_req=$PAYREQ

# Check channel balances
echo Alice channel balance...
sleep $SMALLSTEP
kubectl exec $ALICEPOD -- lncli --network=simnet channelbalance
echo Bob channel balance...
sleep $SMALLSTEP
kubectl exec $BOBPOD -- lncli --network=simnet channelbalance

# Get channel information
echo Getting Alice channel information...
sleep $SMALLSTEP
ALICECHANPOINT=$(kubectl exec $ALICEPOD -- lncli --network=simnet listchannels | jq -r '.channels[0].channel_point')
...
sleep $SMALLSTEP
kubectl exec $BOBPOD -- lncli --network=simnet walletbalance
